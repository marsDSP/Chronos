/**
 * Stage-attributed throughput benchmark for the signal chain, hand-assembled
 * in ChronosEngine::process order. Reports ns per sample per stage in
 * isolation plus the full fused chain, over adaaOrder x mix x blockSize x
 * channels. Flat parameter ramps stand in for the smoothers. Min-of-5 reps.
 * Informational only: exits non-zero on NaN or Inf in the recording pass.
 */
...
#define CHRONOS_NO_VECTORIZE
    ...
    {
        volatile T sink = v;
        (void) sink; // MSVC fallback (weaker; no x64 inline asm)
    }
#endif
    ...
    struct Bufs
    {
        std::vector<float> inL; // test signal
        std::vector<float> inR;
        std::vector<float> dL; // delay out      = drive-gain in
        std::vector<float> dR;
        std::vector<float> vL; // drive out      = adaa in
        std::vector<float> vR;
        std::vector<float> aL; // adaa out       = align wet in
        std::vector<float> aR;
        std::vector<float> wL; // align wet out  = hpf in
        std::vector<float> wR;
        std::vector<float> hL; // hpf out        = lpf in
        std::vector<float> hR;
        std::vector<float> pL; // lpf out        = crossfade wet in
        std::vector<float> pR;
        std::vector<float> yL; // align dry out  = crossfade dry in
        std::vector<float> yR;
        std::vector<float> mL; // crossfade out  = tail in
        std::vector<float> mR;

        void alloc(int n)
        {
            inL.resize(static_cast<std::size_t>(n));
            inR.resize(static_cast<std::size_t>(n));
            dL.resize(static_cast<std::size_t>(n));
            dR.resize(static_cast<std::size_t>(n));
            vL.resize(static_cast<std::size_t>(n));
            vR.resize(static_cast<std::size_t>(n));
            aL.resize(static_cast<std::size_t>(n));
            aR.resize(static_cast<std::size_t>(n));
            wL.resize(static_cast<std::size_t>(n));
            wR.resize(static_cast<std::size_t>(n));
            hL.resize(static_cast<std::size_t>(n));
            hR.resize(static_cast<std::size_t>(n));
            pL.resize(static_cast<std::size_t>(n));
            pR.resize(static_cast<std::size_t>(n));
            yL.resize(static_cast<std::size_t>(n));
            yR.resize(static_cast<std::size_t>(n));
            mL.resize(static_cast<std::size_t>(n));
            mR.resize(static_cast<std::size_t>(n));
        }
    };
    ...
    double stageDrive(const Cfg &c, const Bufs &b, Bufs *rec, float driveLin)
    {
        double acc = 0.0;
        LinRamp ramp;
        ramp.set(driveLin, driveLin, kSamples);
        for (int off = 0; off < kSamples; off += c.block)
        {
    ...
    double stageAdaa(const Cfg &c, const Bufs &b, Bufs *rec)
    {
        ...
        double acc = 0.0;
        ADAA1<TanhNL> a1L, a1R;
        a1L.reset();
        a1R.reset();
        ADAA2<TanhNL> a2L, a2R;
        a2L.reset();
        a2R.reset();
    ...
    {
        double acc = 0.0;
        MarsDSP::Align::SaturatorAlign alL;
        MarsDSP::Align::SaturatorAlign alR;
        alL.reset();
        alR.reset();
    ...
    double stageTail(const Cfg &c, const Bufs &b, Bufs *rec, float gainLin, std::uint32_t &xsL, std::uint32_t &xsR)
    {
        double acc = 0.0;
        LinRamp gainR;
        gainR.set(gainLin, gainLin, kSamples);
        LinRamp bitsR;
        bitsR.set(static_cast<float>(kBits), static_cast<float>(kBits), kSamples);
    ...
    struct FullChain
    {
        ...
        double run(const Cfg &c, const Bufs &b, float driveLin, float gainLin)
        {
            ...
            double acc = 0.0;
            LinRamp driveR;
            driveR.set(driveLin, driveLin, kSamples);
            LinRamp mixR;
            mixR.set(c.mix, c.mix, kSamples);
            LinRamp gainR;
            gainR.set(gainLin, gainLin, kSamples);
            LinRamp bitsR;
            bitsR.set(static_cast<float>(kBits), static_cast<float>(kBits), kSamples);
                ...
                for (int s = 0; s < n; ++s)
                {
                    ...
                    switch (c.mode)
                    {
                        ...
                        case 1:
                            ...
                            if (d1 != nullptr) sat1 = static_cast<float>(adaa1R.
                                                   process(static_cast<double>(drv * wet1)));
                            break;
                        default:
                            sat0 = static_cast<float>(adaa2L.process(static_cast<double>(drv * wet0)));
                            if (d1 != nullptr) sat1 = static_cast<float>(adaa2R.
                                                   process(static_cast<double>(drv * wet1)));
                            break;
                    }
                       ...
                       double &sink)
    {
        ...
        for (int r = 0; r < kReps; ++r)
        {
            ...
            for (int off = 0; off < kSamples; off += c.block)
            {
                std::array<float*, 2> io{
                    eL.data() + off,
                    c.ch > 1 ? eR.data() + off : nullptr
                };
                eng.process(io.data(), c.ch, c.block);
            }
            const auto t1 = Clock::now();
...
int main(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        {
            std::println(stderr, "usage: chain_bench [--csv <path>] [--json <path>] [--provisional]");
            return 2;
        }
    }
    ...
    std::println("=== Chronos chain_bench: stage-attributed end-to-end throughput ===");
    std::println("fs={:.0}  delay={:.1} smp (Lagrange5th)  drive={:.0} dB  gain={:.0} dB  bits={}",
                kFs, static_cast<double>(kDelaySamples), static_cast<double>(kDriveDb),
                static_cast<double>(kGainDb), kBits);
    std::println("hpf={:.0} Hz  lpf={:.0} Hz  Q={:.4}  samples/rep={}  reps={} (min)  arch={}",
                kHpfHz, kLpfHz, kSvfQ, kSamples, kReps, archName());
    std::println("ns per input sample (stereo does 2x the per-sample work of mono).");
    std::println("Isolated stages stream recorded inputs from L3; the fused chain is");
    std::println("cache-resident, so stages-sum overestimates full-chain. Informational only.\n");

    std::println("{:>4} {:>4} {:>4} {:>3} | {:>8} {:>8} {:>8} {:>8} {:>8} {:>8} {:>8} {:>8} | {:>8} {:>8} {:>8}",
                "mode", "mix", "blk", "ch",
    ...
    const std::array<int, 4> blocks = {{ 64, 128, 256, 512 }};
        ...
        for (float mix: mixes)
            for (int block: blocks)
                for (int ch: chans)
                {
                    ...
                    {
                        const std::size_t u = static_cast<std::size_t>(i);
                        b.inL[u] = 0.5f * static_cast<float>(
                                       std::sin(2.0 * kPi * 440.0 * static_cast<double>(i) / kFs));
                        b.inR[u] = 0.5f * static_cast<float>(
                                       std::sin(2.0 * kPi * 330.0 * static_cast<double>(i) / kFs));
                    }
                    ...
                    double sink = 0.0;
                    std::array<double, kNumStages> ns{};
                    ...
                    // delay: prepare outside the timed rep (ring alloc+memset would pollute it)
                    {
                        ...
                        dl.setInterpolation(kInterp);
                        dl.reset();
                        std::vector<float> oL(static_cast<std::size_t>(kMaxBlock)), oR(
                            static_cast<std::size_t>(kMaxBlock));
                        ns[0] = benchNsPerOp([&]()
                                             {
                                                 return stageDelayBody<false>(c, b, nullptr, dl, oL.data(), oR.data());
                                             },
                    ...
                    ns[4] = benchNsPerOp([&]()
                                         {
                                             return stageSvf<false>(c, b.wL, b.wR, nullptr, nullptr,
                                                                    MarsDSP::Filters::SimdSVF::SVFType::HighPass,
                                                                    kHpfHz);
                                         },
                                         static_cast<std::size_t>(kSamples), kReps, sink);
                    ns[5] = benchNsPerOp([&]()
                                         {
                                             return stageSvf<false>(c, b.hL, b.hR, nullptr, nullptr,
                                                                    MarsDSP::Filters::SimdSVF::SVFType::LowPass,
                                                                    kLpfHz);
                                         },
                                         static_cast<std::size_t>(kSamples), kReps, sink);
                    ...
                    std::println("{:4} {:4.0} {:4} {:3} | {:8.3} {:8.3} {:8.3} {:8.3} {:8.3} {:8.3} {:8.3} {:8.3} | {:8.3} {:8.3} {:8.3}",
                    ...
                    for (int st = 0; st < kNumStages; ++st)
                    {
                        csv += archName();
                        csv += ",";
                        csv += std::to_string(mode);
                        csv += ",";
                        csv += std::to_string(static_cast<int>(mix));
                        csv += ",";
                    ...
                    const std::string cfg = "mode=" + std::to_string(mode) + ",mix=" +
                                            ...
                                            ",ch=" + std::to_string(ch);
                    for (int st = 0; st < kNumStages; ++st)
                        records.emplace_back(kStageNames[st], cfg, ns[st]);

                    records.emplace_back("stages-sum", cfg, sum);
                    records.emplace_back("full-chain", cfg, nsFull);
                    records.emplace_back("engine", cfg, nsEngine);
                }
    ...
    std::println("\nrecording pass finite: {}", recordingOk ? "yes" : "NO — CHAIN ASSEMBLY BUG");
    std::println("(sink={})", grandSink);
    std::println("=== DONE (informational only, no pass/fail gate) ===");
