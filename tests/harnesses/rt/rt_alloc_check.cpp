...
namespace
{
    ...
    constexpr std::array<int, 6> kBlockSizes { { 1, 7, 16, 64, 512, 2048 } };
    constexpr int kNumBlockSizes = std::size(kBlockSizes);
...
#define FAIL(...)                                                         \
    ...
    MarsDSP::ChronosEngine::Params sweptParams(int i) noexcept
    {
        ...
        p.loopDrive = lerpF(0.501f, 15.849f, osc(2500.0, 0.0));
    ...
    void printBacktrace()
    {
        ...
        if (g_nFrames > 0)
        {
            std::println("  allocation backtrace ({} frames):", g_nFrames);
            backtrace_symbols_fd(g_frames, g_nFrames, STDERR_FILENO);
        }
#else
        std::println("  (backtrace not available on this platform)");
#endif
    }
} // namespace
...
int main()
{
    std::println("=== Chronos rt_alloc_check ===");
    std::print("fs={:.0} stereo  blocks={}  block sizes: ", kFs, kBlocks);
    for (int i = 0; i < kNumBlockSizes; ++i) std::print("{}{}", kBlockSizes[i], i + 1 < kNumBlockSizes ? "," : "");
    std::println("\n");
    ...
    for (int bi = 0; bi < kNumBlockSizes; ++bi)
    {
        ...
        for (int i = 0; i < kBlocks; ++i)
        {
            ...
            g_armed.store(true, std::memory_order_relaxed);
            std::array<float*, 2> io{ioL.data(), ioR.data()};
            engine.process(io.data(), kChannels, blockSize);
            g_armed.store(false, std::memory_order_relaxed);

            const std::size_t a = g_allocCount.load(std::memory_order_relaxed);
            if (a > 0)
            {
                std::println("FAIL: block size {}, block {}: {} allocation(s) during process",
                            blockSize, i, a);
                std::println("  first alloc: size={} align={}", g_firstSize, g_firstAlign);
                printBacktrace();
                failed = true;
                ++totalFail;
                break;
        ...
        if (!failed)
            std::println("block size {:4}: 0 allocations in {} blocks  PASS", blockSize, kBlocks);
    }

    if (totalFail > 0)
        FAIL("{} block size(s) allocated during process", totalFail);
