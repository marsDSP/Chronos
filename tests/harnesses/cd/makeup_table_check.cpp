/**
 * Correctness harness for the saturator makeup table in SaturatorMakeup.h.
 * Regenerates the table by direct integration and checks the committed
 * constants, the Catmull-Rom interpolant, and the anchor values.
...
int main()
{
    ...
    std::println("=== makeup_table_check ===");
    std::println("integration points = {}, grid = 65, random = 200", kNIntegrate);
    std::println();
    ...
    g_section = "interpolant";
    {
        ...
        std::println("interpolant (200 random): worst err ratio={:.2e} makeup={:.2e} trim={:.2e} (gate 2e-4): PASS",
    ...
    g_section = "anchor values";
    {
        ...
        const std::array<Anchor, 9> anchors = {{
        ...
        for (const auto& a : anchors)
        {
            ...
            if (errR > 1e-5)
                FAIL("anchor {{}} dB rmsRatio: derived {{:.8f}} != spec {{:.8f}} (err {{:.2e}})",
                     a.db, trueRatio, a.ratio, errR);
            if (errM > 1e-5)
                FAIL("anchor {{}} dB outputMakeup: derived {{:.8f}} != spec {{:.8f}} (err {{:.2e}})",
                     a.db, trueMakeup, a.makeup, errM);
            if (errT > 1e-5)
                FAIL("anchor {{}} dB loopTrim: derived {{:.8f}} != spec {{:.8f}} (err {{:.2e}})",
                     a.db, trueTrim, a.trim, errT);
        }
        std::println("anchor values (9 drive levels, gate 1e-5): PASS");
    }
    ...
    g_section = "clamp";
    {
        ...
        std::println("clamp (k<1 and k>16 return table ends): PASS");
    }

    std::println();
    std::println("=== makeup_table_check OK ===");
    return 0;
}
