...
struct Result { std::vector<float> wetL, wetR; };
               ...
               bool stereo)
{
    ...
    std::vector<float> inBlkR, wetBlkR;
    if (stereo) { inBlkR.resize(static_cast<std::size_t>(blk)); wetBlkR.resize(static_cast<std::size_t>(blk)); }
...
double compare(const Result& a, const Result& b, bool stereo, const char* section)
{
    ...
    for (std::size_t i = 0; i < a.wetL.size(); ++i)
    {
        ...
        if (e > static_cast<double>(kTol))
            FAIL("L i=%zu simd=%.9g scalar=%.9g e=%.3e tol=%.0e",
                 i, (double)a.wetL[i], (double)b.wetL[i], e, (double)kTol);
    }
    if (stereo)
    {
        ...
        for (std::size_t i = 0; i < a.wetR.size(); ++i)
        {
            ...
            if (e > maxErr) maxErr = e;
            if (e > static_cast<double>(kTol))
                FAIL("R i=%zu simd=%.9g scalar=%.9g e=%.3e tol=%.0e",
                     i, (double)a.wetR[i], (double)b.wetR[i], e, (double)kTol);
        }
    }
    return maxErr;
}
...
int runAll()
{
    const int blks[] = { 1, 4, 7, 12, 16, 17, 23, 24, 64, 100, 256 };
    ...
    for (int blk : blks)
    {
        for (Interpolation m : modes)
        {
            ...
            if (blk == 256 || blk == 1 || blk == 7)
                std::printf("  blk=%-3d %-4s impulse: max abs diff = %.3e\n", blk, modeName(m), e);
        }
    }
    std::printf("block-size sweep × 3 modes (impulse, D=30): PASS\n");
    ...
    g_section = "fractional delays";
    {
        ...
        const float fracDelays[] = { 30.25f, 30.5f, 30.75f, 99.1f };
        for (float d : fracDelays)
        {
            ...
            std::printf("  D=%.2f ramp: max abs diff = %.3e\n", (double)d, e);
        }
    }
    std::printf("fractional delays (Lag5, ramp): PASS\n");
    ...
    g_section = "stereo";
    {
        ...
        std::printf("  stereo D=40.5: max abs diff = %.3e\n", e);
    }
    std::printf("stereo independence (D=40.5): PASS\n");
    ...
    g_section = "mono";
    {
        ...
        std::printf("  mono D=50.5: max abs diff = %.3e\n", e);
    }
    std::printf("mono path (inR/wetR null): PASS\n");
    ...
    g_section = "ring-wrap";
    {
        ...
        std::printf("  ring-wrap (cap=512, 4000 blocks, ~500 wraps): max abs diff = %.3e\n", e);
    }
    std::printf("ring-wrap endurance: PASS\n");
    ...
    g_section = "delay-move";
    {
        ...
        std::printf("  delay-move (D1=30 -> D2=60, %d blocks): max abs diff = %.3e\n", nBlocks, e);
    }
    std::printf("delay-move crossfade region: PASS\n");
...
int main()
{
    std::printf("=== Chronos SimdDelayLine SIMD-vs-scalar parity harness ===\n");
    std::printf("sr=%.0f  tol=%.0e  (process = SIMD, processScalar = reference)\n\n", kSr, (double)kTol);
