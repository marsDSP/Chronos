...
int main()
{
    ...
    {
        // Same [-π, π] domain as the sinh perf harness for parity.
    ...
    std::cout << "Benchmarking hyperbolic cosine implementations (Block Size: " << blockSize
              << ", Iterations: " << iterations << ")..." << std::endl;

    // 1. Benchmark std::cosh (Scalar Baseline)
    ...
    for (int it = 0; it < iterations; ++it)
    {
        for (int i = 0; i < blockSize; ++i)
        {
            output[i] = MarsDSP::padeCoshApprox(input[i]);
        }
        if (output[0] > 1e9f) std::cout << "Never happens";
    }
    ...
    // Output to CSV
    std::ofstream csv("tests/perf_harness/logs/perf_cosh_results.csv");
    if (!csv.is_open())
    {
        std::cerr << "Failed to open tests/perf_harness/logs/perf_cosh_results.csv" << std::endl;
        return 1;
    }

    csv << "algorithm,avg_time_us,speedup\n";
    csv << std::fixed << std::setprecision(6);
    csv << "std::cosh," << timeStd << ",1.0\n";
    ...
    std::cout << "  std::cosh:       " << std::setw(8) << timeStd    << " us"                                                << std::endl;
