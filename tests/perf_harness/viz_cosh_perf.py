"""Visualise cosh perf benchmark output.

Reads logs/perf_cosh_results.csv (algorithm, avg_time_us, speedup) and produces
...
def main():
    ...
    csv_path = os.path.join(script_dir, "logs", "perf_cosh_results.csv")
    out_path = os.path.join(script_dir, "logs", "perf_cosh.png")

    if not os.path.exists(csv_path):
        print(f"Error: {csv_path} not found. Run perf_cosh_test first (from project root).")
        return
    ...
    ax.set_title("SIMD Pade cosh performance\n(1,000,000 iterations, x in [-pi, pi])")
