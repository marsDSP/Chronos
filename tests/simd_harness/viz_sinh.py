"""Visualise sinh correctness output.

Reads two CSVs:
  - logs/simd_sinh_results.csv:        std vs Pade scalar/SIMD on a [-pi, pi] sweep
  - logs/simd_sinh_adaa_results.csv:   ADAA SIMD vs direct SIMD Pade on a chirp
...
Produces a single PNG with three panels:
  ...
  3) Bottom: ADAA vs direct sinh on a swept-frequency input
"""
...
def main():
    ...
    static_csv = os.path.join(script_dir, "logs", "simd_sinh_results.csv")
    adaa_csv   = os.path.join(script_dir, "logs", "simd_sinh_adaa_results.csv")
    out_path   = os.path.join(script_dir, "logs", "simd_sinh.png")

    if not os.path.exists(static_csv):
        print(f"Error: {static_csv} not found. Run simd_sinh_test first.")
        return

    static = pd.read_csv(static_csv)

    # PASS = SIMD matches scalar within ~single ULP at sinh's value scale.
    # sinh(pi) ~= 11.55 and float eps ~= 1.19e-7, so 1 ULP ~= 1.4e-6;
    ...
    # 1) Function values overlay
    ax_fn = axes[0]
    ax_fn.plot(static["x"], static["std_sinh"],   label="std::sinh (ref)",
    ...
    ax_fn.set_title("sinh(x) over [-pi, pi]")
    ax_fn.set_xlabel("x")
    ax_fn.set_ylabel("sinh(x)")
    ax_fn.legend()
    ...
    if have_adaa:
        ...
        ax_adaa = axes[2]
        ax_adaa.plot(adaa["n"], adaa["direct"], label="direct sinh (Pade SIMD)",
                     color="#3498db", alpha=0.6)
        ax_adaa.plot(adaa["n"], adaa["adaa"],   label="ADAA sinh (SIMD)",
                     color="#9b59b6", alpha=0.8, linewidth=0.9)
        max_diff = float(adaa["diff"].max())
        ax_adaa.set_title(f"ADAA sinh vs direct on a 100Hz->8kHz sweep "
                          f"(max |direct - ADAA| = {max_diff:.3e})")
        ax_adaa.set_xlabel("sample index")
        ax_adaa.set_ylabel("sinh(x[n])")
        ax_adaa.legend(loc="upper right")
        ax_adaa.grid(True, alpha=0.3)

    fig.suptitle(f"sinh correctness  -  {status_text}",
